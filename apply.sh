...
original_author="robgarciab"
original_name="flask_sample"
original_urlname="flask-sample"
original_description="Awesome flask_sample created by robgarciab" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_sample "${name}"
...
then
    ...
    echo "then run 'flask_sample' or 'python -m flask_sample'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_sample' or 'python -m flask_sample' or 'poetry run flask_sample'"
fi
