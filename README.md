...
- 🎯 Entry points to execute your program using `python -m <flask_sample>` or `$ flask_sample` with basic CLI argument parsing.
...
---
# flask_sample Flask Application

Awesome flask_sample created by robgarciab
...
```bash
git clone https://github.com/robgarciab/flask-sample flask_sample
cd flask_sample
make install
```
...
```bash
flask_sample create-db   # run once
flask_sample populate-db  # run once (optional)
flask_sample add-user -u admin -p 1234  # ads a user
flask_sample run
```
